var app = angular.module('matches', ['ngRoute']);
...
app.controller('MainController', function($scope, $routeParams, $http, $timeout, $window) {
    $scope.$on('$routeChangeSuccess', function() {
        ...
        $http.get("/api/v1/matches/" + $routeParams.id + "/users").then(function (response) {
        ...
        $http.get("/api/v1/matches/" + $routeParams.id + "/leagues").then(function (response) {
            $scope.leagues = response.data.data;
        })
    });

    this.delete = function() {
        $http.delete('/api/v1/matches/' + $routeParams.id).then(function(response) {
